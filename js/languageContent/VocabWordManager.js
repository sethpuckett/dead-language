...
export default class {
  ...
  getRandomWord() {
    ...
    let poolIndex = Phaser.Math.RND.between(0, this.availableContent.length - 1)
    let word = this.availableContent.splice(poolIndex, 1)[0]
    this.reservedContent.push(word)
    return word
  }

  releaseWord(word) {
    let index = this.reservedContent.findIndex(r => r.id === word.id)
  ...
  refreshContent() {
    ...
    this.availableContent.sort(this.compareContent)
    this.returnedContent = []
  }

  compareContent(word1, word2) {
